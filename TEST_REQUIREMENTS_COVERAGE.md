...
| 23 | OCR5/6 ML Kit／PP-OCRv6 small・medium・medium + tile比較 | 完了 | 公式PaddleOCR Android SDKとsmall/medium model assetsをvariant別にAPKへ同梱し、共通Gateway／Repository経路、SDK reading order・confidence・4点polygon・不正polygon文字保持、遅延engine cache、variant切替時の旧engine解放、1536px tile・256px overlap・2048px拡大・座標逆変換、edge-clipped判定、面積／IoU／中心距離による入力順非依存の重複統合、DB／Undo非変更、small初期値、選択時非実行、3条件再検出、成功時metadata更新、失敗時前結果保持をunit／Compose／Repository testでカバー。`ppocr-sdk/UPSTREAM.md`に公式commit・URL・SHA-256・licenseを記録。OpenCV 4.5.3のarm64 native pairは公式source＋NDK r28で16 KB LOAD alignmentへ再ビルド済み。2026-08-22に`run-safe-integration-check.cmd`と`run-safe-debug-check.cmd -InstallToDevice`がSuccessし、`PaddleOcrRuntimeSmokeTest`でsmall/medium/medium + tileのoffline推論が完了。同一日本語実画像で3条件を実機再検出し、クラッシュせず結果表示、medium + tileのpolygon重なりを目視確認済み。 |
