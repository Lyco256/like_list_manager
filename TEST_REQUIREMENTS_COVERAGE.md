...
| 22 | OCR3/4全画面ビューア／polygon選択・個別編集／Fit／asset ID／polygon／zoom-pan | 完了 | `OcrViewerGeometryTest` でFit、letterbox、共通zoom/pan座標、viewport再計算、境界clip、不正polygon除外、asset ID対応、内部優先・最小面積、12dp最近傍hit testを確認。`OcrSessionTest` でregion単位の再構成、空region、polygonなしregion、asset ID＋region index編集、structured全文編集禁止、再検出成功／失敗、保存失敗保持を確認。`OcrSessionDialogComposeTest` で単一region入力欄、選択中overlay、全文再構成、選択解除、ページスワイプ・gesture競合、再検出時解除、保存／失敗／再オープンを確認。OCRと表示は同じraw Bitmapデコードを使い、EXIF自動回転差を排除。`run-safe-integration-check.cmd` と`run-safe-debug-check.cmd -InstallToDevice` がSuccess。 |
