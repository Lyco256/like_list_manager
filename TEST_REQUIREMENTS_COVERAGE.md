...
| 21 | メディアグリッド高速スクロールバー | 自動・隔離実機確認済み | `MediaGridScrollbarTest` でordinal位置、端点、最小thumb、clamp、列数2〜12、drag session固定、frame無効化、cancel、最新target、正常完了/取消終了をカバー。request IDとdrag/final種別、UP直後の非dragging、同一target final、古いfinalの新session無効化、完了/取消/consumer cancel後のthumb復帰、bucket開始ordinalのfractionを確認。`MediaGridScrollPositionTest` でtarget ordinalの投稿日/いいねbucket、保存順非表示、同一bucket安定、header開始ordinal索引、日/週/月、特殊bucketを確認。`MediaGridSessionCoordinatorTest` でscrollbar完了checkpointとscroll終了checkpointの重複を防止。`MediaGridScrollbarUiTest` でlabel即時表示、実pointerによる同一bucket固定・bucket越境移動、12dp×4dp・右端配置、UP消去、上部ピル非併用、cancel消去を確認。既存の`MainActivityComposeTest.mediaGridScrollbarJumpsToTheEndAndBackWithoutTakingTheWholeRightEdge` と `MainActivityComposeTest.slowScrollbarReleaseThenNormalGridScrollKeepsThumbFollowingViewport` は維持し、`run-safe-integration-check.cmd` がSuccessとなった。 |
