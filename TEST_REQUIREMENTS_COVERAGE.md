...
| 15 | UI状態・軽微バグ | 一部完了 | 主要tab、dialog、件数、loading、empty、popup、LazyColumn、画面再作成、filter BackHandler破棄、画像viewerの表示/閉じる操作/戻る閉じ、ローカル削除Dialogのcancel/soft delete、概要編集の保存と再作成後復元、投稿者クリックから分類済み投稿者フィルターへの遷移とDB不変、いいね数ポップアップの詳細/暫定警告表示とDB不変、X API設定Dialogの保存/消去導線を確認。細部の追加E2Eを継続 |
