...
| 22 | OCR3全画面ビューア／Fit／asset ID／polygon／zoom-pan | 完了 | `OcrViewerGeometryTest` でFit、letterbox、共通zoom/pan座標、viewport再計算、境界clip、不正polygon除外、asset ID対応を確認。`OcrSessionDialogComposeTest` で全画面UI、1倍ページスワイプ、asset ID一致ページだけのpolygon表示、保存／失敗／再オープンを確認。`run-safe-integration-check.cmd` と `run-safe-debug-check.cmd -InstallToDevice` がSuccess。 |
