...
| 7 | 検索/絞り込み | ロジック完了・UI一部完了 | 全検索target、regex、期間、投稿者、タグ複合、件数summary、非変更。filter dialogのquery適用、投稿者＋タグ条件の複合E2E、Cancel/Back破棄、全クリアE2Eを確認。日付UI E2Eが残る |
| 8 | 分類操作 E2E | 主要項目完了 | 付与・解除、DB・件数、popup非伝播、別グループ同名タグの複数同時付与を確認 |
| 9 | タグ/グループ管理 E2E | 一部完了 | 作成、同名子タグ、tag名称変更/削除、group削除、循環/順序ロジック。group名称変更・代表移動UIが残る |
