...
| 31 | 保存画像の画像重複検索 | 完了 | `LocalImageDuplicateSearchEngineTest`で単一snapshot、0/1件のANN skip、256次元ANN、`min(N,33)`候補、最大32近傍、self除外、strong／mutual threshold、unordered pair dedup、Union-Find連結group、決定的順序、非finite／例外／cancel時closeを確認。`LocalImageDuplicateSearchEngineIntegrationTest`で隔離派生DBと実USearchを人工vectorで接続し、`ClassifiedImageDuplicateSearchViewModelIntegrationTest`でsync待機、NOT_STARTED／FAILED継続、progress、READY、latest generation、FAILED/retry/clearを確認。`ImageDuplicateMediaGridTest`でasset-level順序、parent filter、missing asset省略、同一clip補充なし、通常経路不変を確認し、`ClassifiedSearchUiIntegrationTest`／`UiStateRenderingTest`でDialog apply、専用summary、sort/display無効、filter維持を確認。精度・crop・threshold tuning・実データ目視は対象外。`run-safe-integration-check.cmd` と、その後の `run-safe-debug-check.cmd -InstallToDevice` がSuccess。 |

## 合格ゲート

ローカル検索エンジン／分類済み検索UI: `LocalSearchEngineTest` / `LocalSearchLexicalTest` / `LocalSearchEngineIntegrationTest` でliteral FTS、実768/256 ANN、clip統合、revision再利用・交換、rollback、cancel、closeを固定fixtureで確認する。`ClassifiedSearchViewModelIntegrationTest`、`ClassifiedSearchUiIntegrationTest`、`MainActivityComposeTest` と分類済みロジックテストでSMART/正規表現の状態、表示順、条件AND、検索Dialog、sort無効化、DB非変更を確認する。画像重複検索は`LocalImageDuplicateSearchEngineTest`、`LocalImageDuplicateSearchEngineIntegrationTest`、`ClassifiedImageDuplicateSearchViewModelIntegrationTest`、`ImageDuplicateMediaGridTest`、Compose UI testsで受入条件を確認する。自然言語・画像重複精度・実データ目視は対象外。変更後ゲートは `run-safe-integration-check.cmd` と `run-safe-debug-check.cmd -InstallToDevice`。
