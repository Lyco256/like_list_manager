...
| 23 | OCR5 ML Kit／PP-OCRv6 small比較 | 完了 | 公式PaddleOCR Android SDKとmodel assetsをAPKへ同梱し、共通Gateway／Repository経路、SDK reading order・confidence・4点polygon・不正polygon文字保持、engine別routing、DB／Undo非変更、ML Kit初期値、選択時非実行、選択engine再検出、成功時のみengine／時間更新、失敗時の前結果保持をunit／Compose／Repository testでカバー。`ppocr-sdk/UPSTREAM.md`に公式commit・URL・SHA-256・licenseを記録。OpenCV 4.5.3のarm64 native pairは公式source＋NDK r28で16 KB LOAD alignmentへ再ビルドし、APK内SHA-256／alignmentを確認。`PaddleOcrRuntimeSmokeTest`が16 KB実機上でOpenCV初期化・bundled detector model推論まで成功。2026-08-22に`run-safe-integration-check.cmd`（再実行で全件成功）と`run-safe-debug-check.cmd -InstallToDevice`がSuccess。上書きinstall後、端末offline・既存日本語実画像でPP-OCRv6 smallの文字検出が高精度で成功。 |
