...
- 全header boundaryの開始ordinalからのscrollbar fraction、先頭／末尾clamp、drag session中の固定geometry、高速往復時の最新target、pointer session終了後の見出しピル用metrics破棄を確認します。target ordinal変更は見出し位置一覧の入力ではないことを固定します。
