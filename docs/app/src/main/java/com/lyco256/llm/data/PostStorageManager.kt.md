...
- 現在地の使用容量と移動見積もりは、画像ディレクトリの全走査ではなくRoomの保存済み画像件数・容量集計を使用する
...
- 内部／SDカードのどちらでDBを開く場合もversion 1→2、2→3 migrationを登録する
...
内部・SDカードの全保存先へ `MIGRATION_1_2` と、いいね数列を追加する `MIGRATION_2_3` を登録します。
