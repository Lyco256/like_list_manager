...
比較用検出handlerは要求ごとに`OcrEngine`を受け取ります。空の保存済みOCRを開いたときの自動検出はML Kitです。選択変更だけでは検出せず、再検出は現在選択中のエンジンを使います。成功時だけdraft、構造化結果、表示用のエンジン名・所要時間を同時更新し、失敗や選択変更では直前に成功した結果とメタデータを保持します。選択状態とメタデータはセッション内だけにあり永続化しません。
