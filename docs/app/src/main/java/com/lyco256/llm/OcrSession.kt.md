...
比較用検出handlerは要求ごとに`OcrEngine`を受け取ります。比較画面を開いたときの初期選択はPP-OCRv6 smallです。選択変更だけでは検出せず、再検出は現在選択中のsmall／medium／medium + tileだけを使います。成功時だけdraft、構造化結果、表示用の条件名・所要時間・tile metadataを同時更新し、失敗や選択変更では直前に成功した結果とmetadataを保持します。選択状態とmetadataはセッション内だけにあり永続化しません。
