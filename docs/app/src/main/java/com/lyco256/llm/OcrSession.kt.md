...
`OcrSessionState.qualityMode`は新しいsessionごとに`OcrQualityMode.FAST`へ初期化され、現在のsession keyの間だけ保持されます。Room、Preferences、SavedStateHandle、投稿Entityへ保存しません。保存済み`ocrText`がある投稿は自動検出せず、空の投稿だけ高速モードで1回自動検出します。

モード変更は表示中のstructured result、polygon、draftを変更せず、再検出開始時に選択値を要求へ固定します。検出中・保存中は変更できず、再検出は成功時だけ結果を置き換えます。
