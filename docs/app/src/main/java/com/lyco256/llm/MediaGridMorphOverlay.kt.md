...
- `MediaGridMorphRenderModel` is created once from the session plan and stable Asset-key map. It is withheld until every planned Asset visual and placeholder slot is available; progress does not scan the full item list or rebuild requests.
