...
- primitive-arrayのUnion-Findでedgeの連結成分を作り、孤立assetを除外します。各group内は最強edgeのpairから開始し、既配置endpointへのedge score最大順（同点はasset ID昇順）で追加します。groupは最大edge score降順、最小asset ID昇順です。
