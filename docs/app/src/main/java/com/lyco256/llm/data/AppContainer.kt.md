...
`localSearchEngine`、`localRelatedTweetsEngine`、`imageDuplicateSearchEngine` を各1つ公開する。`localTextEmbedder` は同じ具体的 `LocalTextEmbedder` をSemantic同期とquery検索へ共有し、Japanese CLIPとSudachiも既存instanceを渡す。画像重複engineと関連検索engineは同じ`DerivedSearchStorage`をread-onlyで借用し、それぞれ必要なANN snapshotを所有する。engine生成だけでは辞書・model・ANNをloadせず、Application起動時の検索や関連ANN構築も追加しない。
