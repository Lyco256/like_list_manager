...
- `mediaGridResidentCanvas`はLazyGrid自身の一つのDrawModifierで、layout後の最新layoutInfoからcommandを構築し、resident画像を先に描いてから`drawContent()`します。draw scopeは事前構築済みcommandを順番に描画し、store lookup、state更新、collection生成、IO、ImageRequest、decode、pack readは行いません。
