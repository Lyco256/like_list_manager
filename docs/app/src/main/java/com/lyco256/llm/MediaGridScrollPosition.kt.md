...
- `mediaGridPositionForOrdinal` は同じbucket処理を任意のtarget media ordinalへ適用し、スクロールバー操作終了時の上部現在位置ピルへの引き継ぎに使います。
- `MediaGridHeaderBoundaryIndex` はframe構築時に見出しkey、インライン見出しlabel、開始media ordinalを同じitems走査で作ります。スクロールバー横の全見出しピルはこの事前構築済み索引を使い、pointer MOVE中にheaderやframe全体を再走査しません。保存順では索引が空です。
