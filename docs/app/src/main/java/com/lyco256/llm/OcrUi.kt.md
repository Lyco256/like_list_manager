...
The full-width selector below the header offers ML Kit and PP-OCRv6 small. ML Kit is the default for each newly opened session. Changing the chip only changes the next-request selection; it does not start OCR or relabel the visible result. Redetect uses the selected engine. After a successful result, the viewer shows that result's engine and elapsed seconds; failed detection keeps the previous result and metadata visible. Selector, engine metadata, and timing are never saved.
