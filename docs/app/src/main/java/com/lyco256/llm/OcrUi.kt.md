...
The full-width selector below the header offers PP-OCRv6 small, PP-OCRv6 medium, and PP-OCRv6 medium + tile. PP-OCRv6 small is the default for each newly opened comparison session. Changing the chip only changes the next-request selection; it does not start OCR or relabel the visible result. Redetect uses the selected condition. After a successful result, the viewer shows that result's condition, elapsed seconds, and for tile mode the executed tile and failure counts; failed detection keeps the previous result and metadata visible. Selector, condition metadata, and timing are never saved.
