...
class MediaGridRenderingContractTest {
    ...
    fun viewportAndActiveWindowUseOrdinalBoundariesWithoutLegacyCollections() {
        ...
        val anchor = uiSource.substringAfter("internal fun buildMediaGridViewportAnchorSignature")
            .substringBefore("internal fun MediaGridViewportSignature.toAnchor")
        assertTrue(anchor.contains("MediaGridViewportAnchorSignature"))
        assertTrue(!anchor.contains("visibleItemGeometry"))
        assertTrue(uiSource.contains("visibleMediaGridItemIndices(frame, it)"))
        assertTrue(!uiSource.contains("frame.mediaCellIndices.filter"))
    ...
    fun morphPreparationIsBoundedIdleOnlyAndDoesNotEnableRendering() {
        ...
        val claimPreparation = uiSource.substringAfter("prepareClaimBundle =")
            .substringBefore("isPairReady =")
        assertTrue(!claimPreparation.contains("state.isScrollInProgress"))
        assertTrue(uiSource.contains("state.isScrollInProgress"))
        assertTrue(morphSource.contains("buildMediaGridViewportSignature(layoutInfo, frame, columnCount)"))
        assertTrue(!uiSource.contains("morphViewportSignatureState.value = buildMediaGridViewportSignature"))
        assertTrue(!uiSource.contains("residentPreparedIndex?.drawIndexVersion,\n    ) {\n        combine"))
        val readiness = uiSource.substringAfter("LaunchedEffect(\n        morphPairVersion")
            .substringBefore("effectiveController?.let")
        assertTrue(readiness.contains("snapshotFlow { state.isScrollInProgress }"))
        assertTrue(readiness.contains("morphPointerInProgress"))
        assertTrue(readiness.contains("first { it }"))
    ...
    fun productionRowRendererUsesUniformLatticeInsteadOfEndpointRectLerp() {
        ...
        assertTrue(rendererSource.contains("cell.column * currentCellSize"))
        assertTrue(rendererSource.contains("cell.relativeRow * currentCellSize"))
    ...
    fun morphInteractionSharesOneModifierAcrossTestAndProduction() {
        ...
        assertTrue(uiSource.contains("onFallbackPinchFinished = { anchor, nextColumnCount ->"))
        assertTrue(uiSource.contains("onPinchFinished(anchor, nextColumnCount)"))
        assertTrue(uiSource.contains("MediaGridMorphProductionHandoffEffects("))
        assertTrue(uiSource.contains("mediaGridSingleSurface"))
    ...
    fun productionMorphProtectsFullRowEndpointsAndKeepsNormalDrawAllocationFree() {
        ...
        assertTrue(interactionSource.contains("onCarryoverAssets"))
